...
export function Footer() {
  return (
    <footer style={{ background: 'var(--chrome-bg)', color: 'var(--chrome-fg)', padding: '40px 22px 34px' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '34px 60px' }}>
        <div style={{ flex: '1 1 220px' }}>
          ...
          <p style={{ fontSize: 13.5, lineHeight: 1.55, color: 'var(--chrome-dim)', maxWidth: '34ch', margin: 0 }}>
        ...
        {footerCols.map((col) => (
          <div key={col.head} style={{ flex: '0 1 auto' }}>
            <p style={{ fontFamily: mono, fontSize: 10, letterSpacing: '.16em', textTransform: 'uppercase', color: 'var(--chrome-dim)', margin: '0 0 13px' }}>
              {col.head}
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
              {col.links.map((l) => (
                <Link
                  ...
                  href={l.href}
                  data-nav
                  style={{ textAlign: 'left', color: 'var(--chrome-fg)', fontFamily: "'Instrument Sans', sans-serif", fontSize: 13.5, opacity: 0.85 }}
                >
                  {l.label}
                </Link>
              ))}
