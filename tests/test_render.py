...
# --------------------------------------------------------------------------- #
# Wave 3 — Task 3: generated marker + byte-stable regen + no-dead-link (SITE-06).
# Proves content/rev1/site/ regenerates from render.py with no hand-edited HTML,
# is byte-identical on a double render (no datetime.now()), and has no dead link.
# --------------------------------------------------------------------------- #

_MARKER = "generated by newsletters.render; do not hand-edit"


def test_every_emitted_page_carries_the_generated_marker(tmp_path: pathlib.Path) -> None:
    written = build_site(tmp_path)
    htmls = [p for p in written if p.suffix == ".html"]
    assert {"index.html", "library.html"} <= {p.name for p in htmls}
    for p in htmls:
        assert _MARKER in p.read_text(encoding="utf-8"), f"marker missing in {p.name}"


def test_site_is_byte_stable_across_two_renders(tmp_path: pathlib.Path) -> None:
    a, b = tmp_path / "a", tmp_path / "b"
    wa = build_site(a)
    wb = build_site(b)
    names_a = sorted(p.name for p in wa)
    names_b = sorted(p.name for p in wb)
    assert names_a == names_b, "the two renders produced a different file set"
    for name in names_a:
        ba = (a / name).read_bytes()
        bb = (b / name).read_bytes()
        assert ba == bb, f"{name} is not byte-identical across renders (nondeterminism in HTML)"


def test_rendered_set_matches_pages_and_chrome(tmp_path: pathlib.Path) -> None:
    build_site(tmp_path)
...
def test_no_dead_link_every_internal_href_resolves(tmp_path: pathlib.Path) -> None:
    import re

    build_site(tmp_path)
    rendered = {p.name for p in tmp_path.glob("*.html")}
    # The four nav targets each resolve to a file that exists on disk.
    targets = _nav_targets(_full_site())
    for href in targets.values():
        assert href in rendered, f"nav target {href} has no file on disk"
    # Every internal {slug}.html href in every page points at a file that exists; and no
    # page emits a dead href="None"/empty href anywhere.
    href_re = re.compile(r'href="([^"]*)"')
    for page in tmp_path.glob("*.html"):
        text = page.read_text(encoding="utf-8")
        assert 'href="None"' not in text and 'href=""' not in text, f"dead href in {page.name}"
        for href in href_re.findall(text):
            ...
            if href.endswith(".html") and "://" not in href and "#" not in href:
                if "/" in href:
                    # Cross-corpus Records-strip links (PUB-03) point OUTSIDE this corpus's
                    # directory, so they cannot resolve here by construction. They are not
...
# --------------------------------------------------------------------------- #
# Cross-corpus Records strip (PUB-03, v1.2 Phase 1)
# --------------------------------------------------------------------------- #


def test_records_strip_on_chrome_pages_only(tmp_path: pathlib.Path) -> None:
    """The strip appears on the rev1 chrome pages (Home + Library) and NOWHERE else.

    A reader inside a report is inside ONE record; the cross-record jump belongs at the
    record's front door (01-CONTEXT d2). The strip links point at the assembled published
    tree's other corpora — their existence is proven by the assembled-tree test, their
    SHAPE is pinned here.
    """
    build_site(tmp_path)
    chrome = {"index.html", "library.html"}
    for page in tmp_path.glob("*.html"):
        text = page.read_text(encoding="utf-8")
...
def test_records_strip_empty_renders_nothing() -> None:
    """records omitted/empty → zero strip markup (render.py stays corpus-blind by default)."""
    from newsletters.render import _records_strip, render_home, render_library

    assert _records_strip(None) == ""
    assert _records_strip([]) == ""
    site = _full_site()
    assert 'class="nl-records"' not in render_library(site)
    assert 'class="nl-records"' not in render_home(site)


def test_render_404_is_base_path_absolute_and_marked() -> None:
    """Every href and font url in the 404 page is absolute under base_path (PUB-03).

    GitHub Pages serves 404.html at ARBITRARY depth — a single relative URL here dangles
    for any nested miss. The page must also carry the generated marker (it is renderer
    output like everything else).
    """
    import re

    from newsletters.render import render_404

    html = render_404(base_path="/newsletters/")
    assert "generated by newsletters.render" in html
    assert "url('/newsletters/fonts/" in html and "url('fonts/" not in html
    for href in re.findall(r'href="([^"]*)"', html):
        assert href.startswith("/newsletters/") or "://" in href or href.startswith("#"), (
            f"404 page carries a non-absolute href {href!r}"
        )
    # base_path without a trailing slash normalizes rather than doubling or dropping.
    assert "url('/x/fonts/" in render_404(base_path="/x")
