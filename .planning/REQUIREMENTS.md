...
- [x] **PROB-03** *(hard constraint)*: The Problem layer is a **legibility layer, not an execution tracker** — no write-back to external systems (Jira/Azure DevOps); the problem-SOLVING step stays external/operator-owned (preserves the `semantic.py` spine boundary). Lifecycle-state transitions are human-gated, never auto-mutated
...
| PROB-03 | Phase 13 — Problem Lifecycle Entity | Complete |
