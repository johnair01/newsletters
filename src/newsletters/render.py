...
# The SITE-06 generated-marker sentence, stamped into every rendered page's head (see ``_page``).
# A NAMED constant because it is load-bearing in two places: it proves every committed
# ``content/*/site`` file is renderer output (no hand-editing), and ``publish.assemble_site``
# reads it back as its OWNERSHIP proof before clobbering a previous assembly (WR-05) — two
# copies of that sentence would drift exactly as two normalizers would.
GENERATED_MARKER = "generated by newsletters.render; do not hand-edit"
