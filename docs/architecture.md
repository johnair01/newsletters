...
- **Publish** is deterministic system work: render final surfaces, write to the Library, send
  the weekly per-reader letters. The public GitHub Pages site publishes through exactly ONE
  channel — see §9.
...
## 9. The published record — GitHub Pages, as implemented (v1.2)
...
Chrome pages cross-link the corpora via the Records strip (see `surfaces.md`); sub-corpus
nav/breadcrumbs climb to the site root via `home_href` (no corpus is a dead end).

**The channel.** `.github/workflows/deploy-pages.yml` triggers ONLY on push to `main` (publishing
is human-gated by PR review into main), re-runs the merge-block gate per corpus and the
published-tree guarantees, assembles via the canonical function, and **force-pushes a single
commit to `gh-pages`** naming its source SHA. GitHub Pages serves from that branch. gh-pages is
a *mirror*, not a record — history lives on main. This channel was chosen over
`actions/deploy-pages` because the `github-pages` *environment protection* (a repo setting no
workflow can see) rejected 4/4 prior deploys, including from main (RETRO 2026-07-02; adopting
the environment channel is DEF-14, a maintainer settings decision).

**The guarantees** (`tests/test_publish.py` — PR-blocking in CI's `site-integrity` job AND
re-run pre-publish; one definition of "publishable", no bash re-implementation):

1. every `href`/`src` in the **assembled** tree resolves (cross-corpus links included);
